...
class Neo4jLoader:
    ...
    def set_global_ids(extracted_graph: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        """
        Considering the extracted graph is a list of JSON where each node has a local id, we need to convert the local
...
def extract_graph():
    ...
    pipeline.run(data={'extractor': {'documents': docs[10:23]}}, include_outputs_from={'loader'})
